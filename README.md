...
all messages are signed: messages sent by the canister are certified; messages sent by the client signed by it;
...
Client sends the first message with its client_id and the canister_id it wants to connect to. The message is signed with the private key.
The gateway makes an update call ws_open to the canister with the given id passing on the message. The method returns true if the canister correctly verifies the signature with the previously registered client_id.
- Client composes a message and signs it. Client sends the message to the gateway over the websocket. The gateway makes an update call to forward the message to the canister.
- Canister composes a message and places its hash in the certified data structure. The gateway polls for messages and retrieves the message together with the certificate. The gateway passes on the message and the certificate to the client over the websocket.
...
In many places the code assumes correct arguments being passed and correct order of certain behaviors. Error handling and reliability need to be improved.
Some data might be left over and not properly cleaned up after closing connections. For example, in the current state after all connections to a certain canister are closed, the gateway continues polling for messages.
